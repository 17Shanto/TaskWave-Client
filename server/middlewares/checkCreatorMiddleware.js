...
const checkCreatorMiddleware = async (req, res, next) => {
  try {
    const workspaceId = req.params.id; // Assuming the workspace ID is in the request parameters
    const userId = req.user.id; // Assuming you have stored the user's ID in req.user
